...
/*package*/ class TaskClaimer {
  private static final Logger logger = LoggerFactory.getLogger(TaskClaimer.class);
  ...
  public TaskGenerator claimTask() {
    ...
    logger.debug("Attempting to claim task " + taskRef.getKey() + " on " + ownerId);
  ...
  private TaskGenerator claimTaskInternal() {
    taskRef.runTransaction(new Transaction.Handler() {
      ...
      public Transaction.Result doTransaction(MutableData taskSnpashot) {
        if(interrupted) {
          logger.debug("Claiming task " + taskRef.getKey() + " on " + ownerId + " was interrupted before we started the transaction");
          return Transaction.abort();
        }

        // if this task no longer exists
        if(taskSnpashot.getValue() == null) {
          logger.debug("Tried claiming task " + taskRef.getKey() + " on " + ownerId + " after someone else removed it");
        ...
        if(!(taskSnpashot.getValue() instanceof Map)) {
          logger.warn("Tried claiming task " + taskRef.getKey() + " on " + ownerId + " but it was malformed (" + taskSnpashot.getValue() + ")");
        ...
        if(ourStartState == taskState || (ourStartState != null && ourStartState.equals(taskState))) {
          ...
          logger.debug("Tried claiming task " + taskRef.getKey() + " on " + ownerId + " but its _state (" + taskState + ") did not match our _start_state (" + ourStartState + ")");
          return Transaction.abort();
        }
      }
      ...
      public void onComplete(FirebaseError error, boolean committed, DataSnapshot snapshot) {
        ...
        if(error != null) {
          if(interrupted) {
            logger.debug("Claiming task " + taskKey + " on " + ownerId + " was interrupted during a transaction that errored", error);
            taskLatch.countDown();
          } else if (++retries < FirebaseQueue.MAX_TRANSACTION_RETRIES) {
            logger.debug("Received error while claiming task " + taskKey + " on " + ownerId + "...retrying", error);
            claimTaskInternal();
          }
          else {
            logger.debug("Can't claim task " + taskKey + " on " + ownerId + " - transaction errored too many times, no longer retrying", error);
            taskLatch.countDown();
          }
        }
          ...
          if(interrupted) {
            ...
            logger.debug("Claiming task " + taskKey + " on " + ownerId + " was interrupted during the transaction");
            ...
            logger.debug("Claimed task " + taskKey + " on " + ownerId);
    ...
    try {
      ...
      synchronized (claimLock) {
        if(!claimed) {
          interrupted = true;
          logger.debug("Failed to claim task " + taskRef.getKey() + " on " + ownerId);
          logger.debug("Interrupted while trying to claim a task (" + taskRef.getKey() + ") for " + ownerId);
          logger.debug("Tried claiming task " + taskRef.getKey() + " on " + ownerId + " but we were interrupted");
          return null;
        }
      }
    }
  ...
  public static class TaskGenerator {
    ...
    public Task generateTask(@NotNull String ownerId, @NotNull TaskSpec taskSpec, @NotNull TaskReset taskReset,
                             @NotNull ValidityChecker validityChecker, @NotNull FirebaseQueue.Options options) {
